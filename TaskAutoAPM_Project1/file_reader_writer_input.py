...
Date: 03/22/2024
Program description: This python script has a read function that takes in an argument for a text file and goes line by line,
removes white space and splits the elements of the line by comma and then checks each element if it is a float or not,
makes the conversion between string to float, doubling it, then back to a string and adding it a list named L that gets returned. 

This read function gets called in the write function to get the list of doubled floats then writes it to a new text file in the original format
The main function takes in the name of a file from the command line and passes it into the write function when it is called 
"""
import sys
